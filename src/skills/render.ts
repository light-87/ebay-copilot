 ...
 */
const renderBody = (doc: SkillDoc, baseLevel: number): string => {
 ...
 * the shared body.
 *
 * @param doc - Provider-neutral skill document.
 * @param layer - Skill layer used for the ownership marker.
 * @returns Complete Claude Code `SKILL.md` contents.
 *
 * @example
 * ```ts
 * const markdown = renderClaudeSkill(doc, 'using');
 * ```
 */
export const renderClaudeSkill = (doc: SkillDoc, layer: SkillLayer): string => {
  const frontmatter = [
    '---',
    `name: ${doc.slug}`,
 ...
 * @param doc - Provider-neutral skill document.
 * @param layer - Skill layer used for the ownership marker.
 * @returns Complete Cursor `.mdc` rule contents.
 *
 * @example
 * ```ts
 * const markdown = renderCursorRule(doc, 'contributing');
 * ```
 */
export const renderCursorRule = (doc: SkillDoc, layer: SkillLayer): string => {
  const frontmatter = [
    '---',
    `description: ${yamlString(doc.description)}`,
    'alwaysApply: false',
 ...
 *
 * @param doc - Provider-neutral skill document.
 * @returns Inner Markdown content for a Codex managed block.
 *
 * @example
 * ```ts
 * const section = renderCodexSection(doc);
 * ```
 */
export const renderCodexSection = (doc: SkillDoc): string => renderBody(doc, 2);
